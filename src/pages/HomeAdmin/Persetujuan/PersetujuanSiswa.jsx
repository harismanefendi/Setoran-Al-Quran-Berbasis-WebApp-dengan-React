...
const ApprovalPageSiswa = () => {
  const [teachers, setTeachers] = useState([]);

  useEffect(() => {
    const fetchTeachers = async () => {
      const db = getDatabase();
      const teachersRef = ref(db, "siswa");
      try {
  ...
  const handleApproval = async (uid) => {
    ...
    const db = getDatabase();
    const teacherRef = ref(db, `/$siswa{uid}`);
  ...
  return (
    <div className="mx-auto max-w-screen-lg p-4 overflow-x-auto">
      <h1 className="text-xl md:text-2xl font-bold mb-4">Persetujuan Registrasi Siswa</h1>
...
export default ApprovalPageSiswa;
